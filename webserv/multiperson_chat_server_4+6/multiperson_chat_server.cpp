...
int	get_listener_socket() {
    ...
    hints.ai_family = AF_INET6; // AF_INET for IPv4
...
/** Convert IPv6 address to string manually (C++98 compatible) */
std::string ipv6_to_string(const struct in6_addr& addr) {
    std::ostringstream oss;
    const uint16_t* segments = reinterpret_cast<const uint16_t*>(&addr);
    
    for (int i = 0; i < 8; ++i) {
        if (i > 0) oss << ":";
        oss << std::hex << ntohs(segments[i]);
    }
    return oss.str();
}
...
void	handle_new_connection(int listener, std::vector<pollfd>& pfds) {
	...
	// Step 1: Use sockaddr_storage for generic storage
	// sockaddr_storage can hold IPv4 OR IPv6
	...
	// Step 2: accept() fills remoteaddr with actual client address
	...
	if (newfd == -1) {
		...
		// Pure IPv4 (shouldn't happen with your setup, but just in case)
        if (remoteaddr.ss_family == AF_INET) {
			// It's IPv4! Now we can safely cast to sockaddr_in
            struct sockaddr_in*	addr_in = reinterpret_cast<struct sockaddr_in*>(&remoteaddr);
			//                            ^^^^^^^^^^ Cast to specific type for IPv4 access
            ...
            // IPv6
            struct sockaddr_in6* addr_in6 = reinterpret_cast<struct sockaddr_in6*>(&remoteaddr);
            
			// Check if it's IPv4-mapped IPv6
            if (IN6_IS_ADDR_V4MAPPED(&addr_in6->sin6_addr)) {
                // Extract IPv4 address from IPv4-mapped IPv6
                uint32_t ipv4_addr = *reinterpret_cast<const uint32_t*>(&addr_in6->sin6_addr.s6_addr[12]);
                uint32_t ip = ntohl(ipv4_addr);
                std::string ip_str = ipv4_to_string(ip);
                std::cout << "New IPv4 connection from " << ip_str << " on socket " << newfd << std::endl;
            } else {
                // Pure IPv6
                std::string ip_str = ipv6_to_string(addr_in6->sin6_addr);
                std::cout << "New IPv6 connection from " << ip_str << " on socket " << newfd << std::endl;
            }
